...
def main() -> int:
    ...
    print(f"Cold start (from disk): {cold_secs:.2f}s")
    print(f"Warm run (in memory):    {warm_secs:.2f}s\n")
    ...
    # TODO 8. Estimate what a real evaluation run would cost hosted
    long_row = [r for r in rows if r["case"] == "long"][0]
    # 200 casi per 98 notti (14 settimane * 7 giorni)
    total_multiplier = 200 * (14 * 7) 

    total_prompt = long_row["prompt_tokens"] * total_multiplier
    total_completion = long_row["completion_tokens"] * total_multiplier

    cost_small = estimate(total_prompt, total_completion, tier="small")
    cost_large = estimate(total_prompt, total_completion, tier="large")

    print(f"Estimate (Small tier): {cost_small.summary()}")
    print(f"Estimate (Large tier): {cost_large.summary()}")
